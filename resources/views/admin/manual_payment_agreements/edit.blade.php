                ...
                <div class="card-body">
                    <form action="{{ route('admin.manual-payment-agreements.update', $manualPaymentAgreement->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="content" id="content" rows="12" class="form-control bg-dark text-white border-info @error('content') is-invalid @enderror" required placeholder="Enter the terms, conditions, and disclaimers users must agree to before submitting manual payments...">{{ old('content', $manualPaymentAgreement->content) }}</textarea>
                            @error('content')
