    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card bg-dark bg-opacity-75 border-info rounded-4 p-4 shadow-lg" style="backdrop-filter: blur(10px);">
                <div class="card-body">
                    <form action="{{ route('admin.game_titles.update', $gameTitle->id) }}" method="POST" id="game-title-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="form-text text-secondary mt-2">Recommended size: 1200x400. Leave empty to keep the current image.</div>
                        </div>

                        <div class="mb-4">
                            <label for="description" class="form-label text-muted small text-uppercase fw-bold">Description</label>
                            <textarea name="description" id="description" rows="5" class="form-control bg-dark text-white border-info @error('description') is-invalid @enderror">{{ old('description', $gameTitle->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" id="btn-submit" class="btn btn-lg fw-bold text-white" style="background-color: var(--neon-magenta); border-color: var(--neon-magenta); box-shadow: 0 0 15px rgba(255, 0, 255, 0.5);">
