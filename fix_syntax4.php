...
// The modals have remnants of Livewire variables. We replaced the entire blocks via Python script earlier, but that was wiped out when I git reset the file!
// Let's re-apply the correct search modal replacement.

$marshallModalHtml = <<<HTML
...
$html = preg_replace('/<label class="form-label">MARSHALL USERNAME<\/label>.*?<div class="modal-footer border-0 pt-0">\s*<button type="button" class="btn btn-neon w-100" style="border-color: #ffdd00; color: #ffdd00;" wire:click="electMarshall\(\)" data-bs-dismiss="modal">ELECT USER<\/button>\s*<\/div>/s', $marshallModalHtml, $html);
...
$html = preg_replace('/<label class="form-label">PLAYER USERNAME<\/label>.*?<div class="modal-footer border-0 pt-0">\s*<button type="button" class="btn btn-neon w-100" wire:click="sendInvite\(\)" data-bs-dismiss="modal".*?SEND INVITE<\/button>\s*<\/div>/s', $inviteModalHtml, $html);
